...
def main():
    ...
    mask_filtered = (df['is_holding'] == False) & (df['pct_chg'] > 2.0) & (df['vol'] > 0)
    ...
    result = {
        "timestamp": time.time(),
        "news_catalyst": news,
        "valid_targets": df_final[['stock_code', 'price', 'pct_chg', 'vol', 'is_holding']].to_dict(orient='records')
    }
    
    # 修复了 OUTPUT_PATH 的命名错误
    try:
